...
and records them to a log.  It prints each received payload on the
console along with the packet RSSI and the current noise RSSI, and it
writes a CSV log with ISO timestamps for later analysis.

The script relies on the same ``sx126x`` driver as the transmitter.  It
assumes the LoRa HAT is connected to ``/dev/serial0`` and that the
frequency, address and air speed match those of the transmitter.  If
your settings differ, adjust the configuration section below.

Unlike the transmitter, no modifications are needed to the TX script
to capture RSSI: RSSI is measured at the receiver.  The SX126x
driver prints RSSI values when a packet is received.  This script
parses those printed lines to extract both the packet RSSI and the
ambient noise RSSI.
"""
...
MY_ADDR = 2                     # This node’s address; must match TX peer
...
def listen() -> None:
    """Main loop: initialise the radio and print/log packets."""
    ensure_driver()
    try:
    ...
    while True:
        ...
        if out:
            ...
            if payload_raw is not None:
                ...
                ts = now_iso()
                # Log to file
                csv_line = f"{ts},{packet_rssi},{noise_rssi},{payload}\n"
